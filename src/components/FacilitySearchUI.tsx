...
export default function FacilitySearchUI({
  ...
  return (
    <Draggable handle=".drag-handle" nodeRef={nodeRef}>
      <div className="absolute top-14 sm:top-4 left-1/2 -translate-x-1/2 z-10 flex-col sm:flex items-center space-y-2 gap-2 sm:space-y-0 bg-white p-2 rounded-lg shadow max-w-[95%]">
        <div
          className="drag-handle w-10 h-1 bg-gray-300 rounded cursor-grab active:cursor-grabbing mb-2"
          title="Drag me"
          ref={nodeRef}
        />
        <Input
        ...
        />
        {errorMessage && (
          <div className="text-red-500 text-[12px] text-right absolute top-12">
            {errorMessage}
          </div>
        )}
        ...
        <div
          className={`grid grid-cols-2 gap-2 ${
            errorMessage && "mt-10 md:mt-4 sm:mt-4"
          }`}
        >
          <Button
          ...
          >
            ...
            {isLoading && (
              <ImSpinner2
                className="animate-spin text-gray-200 ml-2"
                size={20}
              />
            )}
          </Button>
